...
def draft_communications(state: AgentState) -> AgentState:
    ...
    # Generate suggested actions (what will happen if approved)
    suggested_actions = []
    for email in emails:
        suggested_actions.append({
            'action': 'send_email',
            'description': f"Send crisis response email to {email['recipient_role']}",
            'recipient': email['recipient_role']
        })

    suggested_actions.append({
        'action': 'update_schedule',
        'description': f"Allocate dock for {state['vessel_name']} - {recommended['title']}",
        'duration_days': recommended.get('duration_days', 0)
    })

    suggested_actions.append({
        'action': 'log_decision',
        'description': f"Log crisis resolution decision in system",
        'option_selected': recommended.get('option_number', 1)
    })

    suggested_actions.append({
        'action': 'notify_team',
        'description': f"Alert shipyard team of emergency response plan",
        'priority': 'high'
    })

    state['suggested_actions'] = suggested_actions
    state['messages'].append(f"Prepared {len(suggested_actions)} actions for approval")
